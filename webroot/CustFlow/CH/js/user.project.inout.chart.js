...
$(function () {
	...
	$("button.btn").click(function () {
	    ...
	    if (theme == "chartclear") {
	        ...
	        if (theme == "inoutflow") {
	            if ($("#inoutChart").html() == "详细进出柱图") {
	                SerchResultData.length = 0;
	                //preDeviceGuid = "";
	            }

	            $("#inoutChart").html("添加报表");
	        }
	        else {
	            $("#inoutChart").html("详细进出柱图");
	           // preDeviceGuid = "";
			...
			.get(
					...
					function (data, textStatus) {
					    ...
					    if (textStatus != "success") {
					        ...
					        $("#inout_Error")
									.append(
											"<div id='inout_Errordiv'>请求错误</br>请<a href='javascript:DrawFlow();'class='pagecolor'>刷新</a></div>");
					        return;
					    }
					    if (data === "ERROR") {
					        $("#inout_Shadow").hide();
					        ...
					        $("#inout_Error")
									.append(
											"<div id='inout_Errordiv'>后台发生错误</br>请<a href='javascript:DrawFlow();'class='pagecolor'>刷新</a></div>");
					        return;
					    }
					    try {
					        var myobj = eval('(' + data + ')');
					        ...
					        $("#inout_Error")
									.append(
											"<div id='inout_Errordiv'>数据转换失败,格式不正确</br>请<a href='javascript:DrawFlow();'class='pagecolor'>刷新</a></div>");
					    ...
					    if (myobj == undefined) {
					        ...
					        $("#inout_Error")
									.append(
											"<div id='inout_Errordiv'>未获取到数据</br>请<a href='javascript:DrawFlow();'class='pagecolor'>刷新</a></div>");
    ...
    for (var i = 0; i < SerchResultData.length; i++)
    {
        ...
        if (i % 2 == 0)
        {
         var curName = curData.name;
            titleStr += curName.substr(0,curName.length-2)  + " 合计：In " + vNum + "人 ";
            }
        else
            titleStr += "Out " + vNum + "人   ";
