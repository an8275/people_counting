...
function getPeoFlowAndSalesInfo() {
	...
	if(startTime==''||endTime==''){
		var vTime;
		var myDate = new Date();
        var myNewYear = myDate.getFullYear();    //获取完整的年份(4位,1970-????)
        var myNewMonth = myDate.getMonth();       //获取当前月份(0-11,0代表1月)
        var myNewDay = myDate.getDate();        //获取当前日(1-31)
		if (idName == "selectDay")//提取最近三天
		{
			    ...
			    $.get(
                        ...
                        function (data, textStatus) {
                            if (textStatus != "success") {
                                ...
                                $("#CustListTable_Error")
                                        .append(
                                                "<div id='CustListTable_Errordiv'>Error!!</br>Please<a href='javascript:getPeoFlowAndSalesInfo();'class='pagecolor'>refresh the page</a></div>");
						//alert("请求错误,请刷新");
						return;
                            }
                            if (data === "ERROR") {
                                ...
                                $("#CustListTable_Error")
                                        .append(
                                               "<div id='CustListTable_Errordiv'>Background support run error!!</br>Please<a href='javascript:getPeoFlowAndSalesInfo();'class='pagecolor'>refresh the page</a></div>");
						//alert("后台发生错误,请刷新");
						return;
                            }
                            try {
                                ...
                                $("#CustListTable_Error")
                                        .append(
                                                "<div id='CustListTable_Errordiv'>Data conversion failed, wrong page format!!</br>Please<a href='javascript:getPeoFlowAndSalesInfo();'class='pagecolor'>refresh the page</a></div>");
						...
						return;
                            ...
                            if (mySaleObj == undefined) {
                                ...
                                $("#CustListTable_Error")
                                        .append(
                                                "<div id='CustListTable_Errordiv'>Data accessing failed!!</br>Please<a href='javascript:getPeoFlowAndSalesInfo();'class='pagecolor'>refresh the page</a></div>");
						//alert("未获取到数据！请刷新");
						return;
                            }
                            ...
                            $('#personChart').highcharts({
                                ...
                                title: {
                                    text: 'Data in the last 3 '+ nTitleGap + 's'
                                },
                                xAxis: [{
...
$(function () {
    ...
    var currentDateTime = new Date();
    $("#showCurDate").html(currentDateTime.getFullYear() + '-' + (currentDateTime.getMonth() + 1) + '-' + currentDateTime.getDate());
    ...
    if(getcookievalue("UserProject")=="上海昊亿实业有限公司"){
		$("#projectName").html("Highilght Manufacturing Corp.,Ltd");
	}else{
