		...
		if(theme=="personflow"){//客流柱图
           ...
           if(checkBox.length>0){
		       ...
		       for (var i = 0; i < checkBox.length; i++) {
				   ...
				   DrawPersonFlow();
				   detil+="<div >"+areaname+"&nbsp;&nbsp;&nbsp;total people flow:<label style='font-size:20px;color: red;'>"+perperson+"</label></div></br>";
				   $("#detil").html(detil);
			...
			detil+="<div >"+areaname+"&nbsp;&nbsp;&nbsp;total people flow:<label style='font-size:20px;color: red;'>"+perperson+"</label>&nbsp;&nbsp;&nbsp;Sales("+currencytype+")：<label style='font-size:20px;color: red;'>"+perSale+"</label></div></br>";
		    $("#detil").html(detil);
			$("#detilbox").show();
			...
			DrawPersonFlow();
			detil+="<div >"+areaname+"&nbsp;&nbsp;&nbsp;total people flow:<label style='font-size:20px;color: red;'>"+perperson+"</label>&nbsp;&nbsp;&nbsp;Orders：<label style='font-size:20px;color: red;'>"+perOrder+"</label></div></br>";
		    $("#detil").html(detil);
			$("#detilbox").show();
			...
			.get(
					...
					function (data, textStatus) {
					    ...
					    if (textStatus != "success") {
					        ...
					        $("#flow_Error")
									.append(
											"<div id='flow_Errordiv'>Error!!</br>Please<a href='javascript:DrawPersonFlow();'class='pagecolor'>refresh the page</a></div>");
						//alert("请求错误,请刷新");
						return;
					    }
					    if (data === "ERROR") {
					        ...
					        $("#flow_Error")
									.append(
											"<div id='flow_Errordiv'>Background support run error!!</br>Please<a href='javascript:DrawPersonFlow();'class='pagecolor'>refresh the page</a></div>");
						//alert("后台发生错误,请刷新");
						return;
					    }
					    try {
					        ...
					        $("#flow_Error")
									.append(
											"<div id='flow_Errordiv'>Data conversion failed, wrong page format!!</br>Please<a href='javascript:DrawPersonFlow();'class='pagecolor'>refresh the page</a></div>");
					    ...
					    if (myobj == undefined) {
					        ...
					        $("#flow_Error")
									.append(
											"<div id='flow_Errordiv'>Data accessing failed!!</br>Please<a href='javascript:DrawPersonFlow();'class='pagecolor'>refresh the page</a></div>");
						//alert("未获取到数据！请刷新");
						return;
					    };
                        salesItems = myobj;
...
function GenData() {
    ...
    if (theme == "andsalesrate") {
        ...
        allSale+=perSale;
        vSRdata = {
            name: salesname+"：Average expenditure",
            data: vPersonSales,
            color: createRandnColor(),
            yAxis: 1,
            type: 'spline'
    ...
    if (theme == "andsaleaverage") {
        ...
        allOrder+=perOrder;
        vSRdata = {
            name: salesname+"：Rate of purchase",
            data: vPersonOrders,
            color: createRandnColor(),
            yAxis: 1,
            type: 'spline'
...
function DrawPersonFlow() {
    ...
    if (theme == "personflow")
        chartText = "people flow amount：" + allPerson;

    if (theme == "andsalesrate")
        chartText = "people flow amount：" + allPerson + "---- sales volum amount("+currencytype+")：" + allSale ;

    if (theme == "andsaleaverage")
        chartText = "people flow amount：" + allPerson + "---- order number amount：" + allOrder ;
...
function personflowdata(){
	...
	if(Methods=='month'){		
		...
		//alert('date '+personItems['column']);
		//alert('date '+personItems['outall']);
		//alert('date '+personItems['inall']);
